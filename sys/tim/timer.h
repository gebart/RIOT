 ...
 * @return [description]
 */
int timer_on_overflow(tim_t dev, void (*callback)(void*));

int timer_set_cb(tim_t dev, uint8_t chan, timer_cb, void *arg);
 ...
 */
int timer_set_rel(tim_t dev, int channel, unsigned int timeout);
 ...
 * @return                  1 on success, -1 on error
 */
int timer_set_abs(tim_t dev, int channel, unsigned int value);
